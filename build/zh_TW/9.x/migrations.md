...
> **Warning** default 陳述式的支援程度會依據資料庫 Driver、資料庫版本、欄位型別等而有所不同。請參考所使用的資料庫之說明文件。此外，在使用 `change` 方法更改欄位時，無法使用原始的 `default` 陳述式 (即，使用 `DB::raw`)。
...
> **Warning** 使用 Microsoft SQL Server 時，請確保有安裝 `doctrine/dbal:^3.0`。
...
> **Warning** 可修改的欄位型別有：bigInteger`, `binary`, `boolean`, `char`, `date`, `dateTime`, `dateTimeTz`, `decimal`, `double`, `integer`, `json`, `longText`, `mediumText`, `smallInteger`, `string`, `text`, `time`, `tinyText`, `unsignedBigInteger`, `unsignedInteger`, `unsignedSmallInteger`, 與 `uuid`。若要修改 `timestamp` 欄位型別，則[必須先將其註冊為 Doctrine 型別](#prerequisites)。

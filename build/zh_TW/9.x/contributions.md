...
為了鼓勵活躍的社群寫作，Laravel 不建議只進行 Bug 回報，更強烈建議大家提供 Pull Request。Pull Request 只有在標註為「Ready for review (可供檢閱)」 (即不在「Draft (草稿)」狀態) 且新功能的所有測試例都通過時才會被檢閱。長期放置、非活躍的「Draft (草稿)」 Pull Request 會在幾天內被關閉。
...
對於目前版本有**完整向下相容性**的**次要**功能，可以送交至最新的穩定分支 (目前為 `9.x`)。

**主要**新功能，或是包含中斷性變更的修改，則應送交至 `master` 分支，這個分支包含了未來的版本。

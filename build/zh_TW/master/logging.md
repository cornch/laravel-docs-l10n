...
An `ErrorLogHandler` based Monolog driver `monolog` | A Monolog factory
...
`stack` | A wrapper to facilitate creating "multi-channel" channels `syslog`
...
their features have been deprecated and will be removed in a future
version. If you would like to log these deprecation warnings, you may
specify your preferred `deprecations` log channel in your application's
...
application's `logging` configuration file. To accomplish this, you may pass
a configuration array to the `Log` facade's `build` method:
...
to customize (or "tap" into) the Monolog instance after it is created. There
is no conventional location where these classes should be placed, so you are
free to create a directory within your application to contain these classes:
...
instance. The `Illuminate\Log\Logger` instance proxies all method calls to

...
為了鼓勵活躍的社群寫作，Laravel 不建議只進行 Bug 回報，更強烈建議大家提供 Pull Request。「Bug 回報」也可以通過 Pull
Request 的形式來提供——在該 Pull Request 中包含失敗的測試例。Pull Request 只有在標註為「Ready for
review (可供檢閱)」 (即不在「Draft (草稿)」狀態) 且新功能的所有測試例都通過時才會被檢閱。長期放置、非活躍的「Draft (草稿)」
Pull Request 會在幾天內被關閉。
...
- [Discord](https://discord.gg/laravel)- [Larachat](https://larachat.co)
...
可以在 Laravel 框架存放庫的 [GitHub Discussion
Board](https://github.com/laravel/framework/discussions) 上提議新功能或是改進現有的
Laravel 行為。若要提議新功能，請至少準備好要能實作完成此功能所需的部分程式碼。

有關 Bug、新功能、以及現有功能實作的一些比較不正式的討論，會在 [Laravel Discord
伺服器](https://discord.gg/laravel)上的 `#internals` 頻道內進行。Taylor Otwell ——
Laravel 的維護人員，通常會在工作日的上午 8 時至下午 5 時 (UTC-06:00 或 America/Chicago)
或是其他不定期時間出現在此頻道內。
...
若不確定你的功能是主要功能還是次要功能，請在 [Laravel Discord 伺服器](https://discord.gg/laravel)的
`#internals` 頻道內詢問 Taylor Otwell。
...
- 可被合理視為騷擾的行為不會被容忍。
   Behavior that can be reasonably considered harassment will not be
tolerated.

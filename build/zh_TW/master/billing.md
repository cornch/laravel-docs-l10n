...
ships with Laravel. If you wish to change this you may specify a different
...
subscribed to the application's "premium" product. The given Stripe product
identifier should correspond to one of your product's identifiers in the
Stripe dashboard:
...
that has a base subscription price of $10 per month but offers a live chat
add-on price for an additional $15 per month. Multiprice subscription
...
`success` (integer) query string variables will be added to the URL. The
...
to use the `Stripe\StripeClient` client directly. For example, you could use
this method to access the `StripeClient` instance and retrieve a list of

...
them, it is best to use the `lazyById` or `lazyByIdDesc` methods
instead. These methods will automatically paginate the results based on the
record's primary key:
...
clause into your query. These methods accept an optional array of bindings
...
string as the value of the "having" clause. These methods accept an optional
...
receive a `Illuminate\Database\Query\JoinClause` instance which allows you
to specify constraints on the "join" clause:
...
query. The most basic call to the `where` method requires three
...
instance, allowing you to define the query that should be placed inside of
the "exists" clause:
...
results of a subquery. You may accomplish this by passing a column,
...
The `whereFullText` and `orWhereFullText` methods may be used to add full
text "where" clauses to a query for columns that have [full text
indexes](/docs/{{version}}/migrations#available-index-types). These methods
will be transformed into the appropriate SQL for the underlying database
system by Laravel. For example, a `MATCH AGAINST` clause will be generated
for applications utilizing MySQL:
...
indicating the columns to be updated. The `update` method returns the number
of affected rows. You may constrain the `update` query using `where`
clauses:
...
statement with a "shared lock", you may call the `sharedLock` method. A

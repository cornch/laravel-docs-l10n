...
可以傳入一個閉包給這些 Assertion 方法來判斷某個 Job 是否通過給定的「真值測試 (Truth Test)」。若分派的 Job 中至少有一個 Job 有通過給真值測試，則 Assertion 會被視為成功。舉例來說，我們可以判斷是否有對某個特定訂單分派 Job：
...
使用 `Process` Facade 的 `fake`，就可以讓 Laravel 的 Process 服務在執行 Process 時回傳 ^[Stubbed](預先填充好的)、假的結果。更多有關模擬 Process 的資訊，請參考 [Process 的測試說明文件](/docs/{{version}}/processes#testing)。

<a name="queue-fake"></a>

## Queue Fake

可以使用 `Queue` Facade 的 `fake` 方法來避免放入佇列的 Job 被推入到[佇列](/docs/{{version}}/queues)中。通常來說，這麼做就可以判斷 Laravel 有被指示要將給定的 Job 推入到佇列中了。因為，我們可以在其獨立的測試 Class 中測試放入佇列的 Job。
...
> **Note** 若要測試使用 `Bus` Facade 的批次或串聯 Job，請參考 `Bus` Facade 的[批次 Assertion](#bus-job-batches)說明文件，以及 [串聯 Assertion](#bus-job-chains)說明文件。

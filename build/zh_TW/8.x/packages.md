...
may load the `admin` view from the `courier` package like so:
...
define two publish groups (`config` and `migrations`) in the `boot` method
of a package's service provider:

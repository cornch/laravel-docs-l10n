...
若想使用 Migration 來完整重設資料庫，請使用 `Illuminate\Foundation\Testing\DatabaseMigrations`。不過，`DatabaseMigrations` Trait 比起 `RefreshDatabase` Trait 來說會慢很多。
...
通過 `fake` 輔助函式，Factory 就可以存取 [Faker](https://github.com/FakerPHP/Faker) PHP 函式庫。該函式庫可用來方便地產生各種類型的隨機資料以進行測試。

> **Note** 可以通過在 `config/app.php` 組態設定檔中加上 `faker_locale` 選項來設定專案的 Faker 語系設定。

...
使用者被重新導向到登入驗證 ^[Callback](%E5%9B%9E%E5%91%BC) Route 後，就可以使用 Socialite 的 `user` 方法來取得使用者的詳細資料。`user` 方法回傳的使用者物件提供了多種屬性與方法，我們可以將與該使用者有關的資訊存在資料庫中。根據所使用的 OAuth Provider 支援的是 OAuth 1.0 還是 OAuth 2.0，該物件上會有不同的屬性與方法：
...
#### 以 ^[Token](%E6%AC%8A%E6%9D%96) 來取得使用者詳細資料 (OAuth2)
...
#### 以 ^[Token](%E6%AC%8A%E6%9D%96) 與 ^[Secret](%E5%AF%86%E9%91%B0) 來取得使用者詳細資料 (OAuth1)
